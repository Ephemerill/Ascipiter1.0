...
def _parse_item_paragraph(p_tag) -> dict | None:
    ...
    Returns {'meal': ..., 'description': ...} or None if no name could be extracted.
    """
    ...
    return {"meal": meal_name, "description": description}
...
def _scrape_structured_menu(url: str, target_stations: list) -> dict:
    ...
    if day_sections:
        ...
        structured_menu = _parse_meal_type_sections(day_sections, normalized_target_stations)
        if not structured_menu:
            logging.warning("New-layout parsing finished, but no items found for target stations.")
        else:
